...
value1 = sns.distplot(RPar1).get_lines()[0].get_data()
...
sns.distplot(RPar2,color='red')
value2 = sns.distplot(RPar2).get_lines()[0].get_data()
...
value1 = sns.distplot(GPar1).get_lines()[0].get_data()
maxPar1 = value1[0][np.argmax(value1[1])]
plt.title("MAP = %2.4f" % maxPar1, fontsize=12)
plt.xlabel('Bias')
plt.ylabel('Frequency')
...
plt.subplot(122)
sns.set_style('darkgrid')
sns.distplot(GPar2,color='green')
value2 = sns.distplot(GPar2).get_lines()[0].get_data()
maxPar2 = value2[0][np.argmax(value2[1])]
